...
portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
	...
	if( ulTimeOut != 0UL )
	{
		configASSERT( InsideISR() == ( portBASE_TYPE ) 0 );
...
err_t sys_sem_new_n(sys_sem_t *pxSemaphore, u8_t maxCount, u8_t ucCount)
{
	err_t xReturn = ERR_MEM;
	*pxSemaphore = (sys_sem_t *)xSemaphoreCreateCounting(maxCount,ucCount);
	if( *pxSemaphore != NULL )
	{
		xReturn = ERR_OK;
		SYS_STATS_INC_USED( sem );
