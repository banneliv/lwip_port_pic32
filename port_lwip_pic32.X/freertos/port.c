 ...
 */
								extern	void __ISR(configTICK_INTERRUPT_VECTOR,ipl1) vPortTickInterruptHandler( void );
//extern void __attribute__( (interrupt(IPL2SOFT), vector( configTICK_INTERRUPT_VECTOR ))) vPortTickInterruptHandler( void );
 ...
 */
										void __ISR(_CORE_SOFTWARE_0_VECTOR,ipl1) vPortYieldISR( void );
//void __attribute__( (interrupt(IPL1SOFT), vector(_CORE_SOFTWARE_0_VECTOR))) vPortYieldISR( void );
...
// Allready done by the pragma config
...
void vAssertCalled( const char *pcFileName, unsigned long ulLine){
	...
	{
		printf("ASSERT ERROR : file %s, at line %d, before start of scheduler\r\n",pcFileName,ulLine);
	}
	else
	{
		printf("ASSERT ERROR : file %s, at line %d, from task %s\r\n",pcFileName,ulLine,pcTaskGetTaskName(NULL));
	}
}
